...
class StatementParser:
    ...
    def create_spending_plots(self, df):
        try:
            ...
            fig_bar.update_layout(
                title='Spending by Category',
                **layout_settings,
                height=max(250, len(df) * 40),  # Dynamic height based on number of categories
                xaxis={
                    'tickfont': {'size': 10},
                    'showgrid': True,
            ...
            fig_pie = go.Figure(
                data=[go.Pie(
                    labels=df['category'],
                    values=df['amount'].abs(),  # Use absolute values
                    hole=0.4,
                    textinfo='none',  # Hide text on pie
            ...
            fig_pie.update_layout(
                ...
                **layout_settings,
                ...
                annotations=[{
                    'text': f'Total<br>₹{df["amount"].abs().sum():,.0f}',
                    'x': 0.5,
                    'y': 0.5,
